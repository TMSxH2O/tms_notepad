...
> TODO 调整逻辑
> 
> 1. 最前面可以简单说一下模组的定义或者这个说法的出处
> 2. 以Mod为卖点的游戏，他们的做法本质上区别都不会很大，但是这里直接从Minecraft这种成熟的例子讲起，后面就基本上没内容了，换一下（先老滚5，再MC，中间可以插个别的，凑三个）
> 3. 讲讲steam创意工坊到底会做些什么，和游戏模组有什么具体的关系
> 4. 游戏启动器（部分原本不支持mod的游戏，安装mod的手段之一）一般是怎么做的（这个内容有点多有点深，基本上就需要讲hook都是怎么做的，准备不充分讲了估计就是丢人）
> 5. 
...
> 更新到现在，微软依然没有提供相关模组制作工具的想法，然而网易代理的版本很早就出推出了模组工具 MC Studio（地图编辑、事件蓝图、UI等常用功能）
