...
void CGroundPass::updateV()
{
	...
	const auto& OcclusionPosition = ElayGraphics::ResourceManager::getSharedDataByName<glm::vec3>("OcclusionPos");
	m_pShader->setFloatUniformValue("u_occlusion_pos", OcclusionPosition.x, OcclusionPosition.y, OcclusionPosition.z);
	const auto& OcclusionRadius = ElayGraphics::ResourceManager::getSharedDataByName<float>("OcclusionRadius");
	m_pShader->setFloatUniformValue("u_occlusion_radius", OcclusionRadius);

	glBindVertexArray(ElayGraphics::ResourceManager::getGameObjectByName("Ground")->getVAO());
	glDrawArrays(GL_TRIANGLES, 0, 6);
	glBindVertexArray(0);

	m_pShader->setMat4UniformValue("u_ModelMatrix", glm::value_ptr(ElayGraphics::ResourceManager::getGameObjectByName("Occlusion")->getModelMatrix()));
	drawSphere();
