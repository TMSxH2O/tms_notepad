...
3. 设置帧缓冲附件
    ...
    - 纹理附件
		...
		如，某个面片着色器中存在4个输出（可以使用 `layout (location = {索引}) out` 来进行指定，之后的输出将会按照索引的顺序），此时只需要后三项输出，可以指定 `glDrawBuffers` 的第一项为 `3`，第二项参数传入 `GL_NONE, GL_ATTACHMENT0, GL_ATTACHMENT1, GL_ATTAHMENT2`
