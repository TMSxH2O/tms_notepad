...
vec3 integrateLTC(vec3 vNormal, vec3 vViewDir, vec3 vFragPos, mat3 vLTCMatrix, vec3 vPolygonalLightVertexPos[4], bool vTwoSided, out vec3 LookupVector)
{	
	//着色点上的切线空间正交基
...
bool InsectSphere(vec3 ray_ori, vec3 ray_dir, vec3 pos, float radius)
{
	vec3 od = pos - ray_ori;
	float dis = length(cross(od, normalize(ray_dir)));
...
void main()
{
	...
	vec3 LookupVector = vec3(0, 0, 0);
	vec3 Diffuse = integrateLTC(GroundNormal, ViewDir, v2f_FragPosInWorldSpace, mat3(1), u_PolygonalLightVertexPos, u_IsTwoSided, LookupVector);

	if (InsectSphere(v2f_FragPosInWorldSpace, LookupVector, u_occlusion_pos, u_occlusion_radius)) {
		Diffuse = vec3(0);
	}

	//vec3 Specular = integrateLTC(GroundNormal, ViewDir, v2f_FragPosInWorldSpace, LTCMatrix, u_PolygonalLightVertexPos, u_IsTwoSided, LookupVector);
